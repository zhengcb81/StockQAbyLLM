...
基于 httpx 的异步 LLM API 提供者，支持连接池和并发请求。
"""
...
class AsyncLLMProvider(SearchProvider):
    """使用异步 LLM API 的提供者。

    通过通用提示词让LLM分析任意公司并生成评分（1-10）和详细描述。
    支持多个LLM提供商和自动故障切换。
    使用 httpx 异步客户端，支持连接池和并发请求。
    """
    ...
    def __init__(
        ...
        logger.info(
            f"AsyncLLMProvider 初始化完成 (provider: {self.provider_name}, " f"model: {self.model})"
        )

    def search(self, query: str) -> List[Dict[str, Any]]:  # type: ignore[override]
        """同步搜索方法（兼容接口）。

        注意：此方法会运行异步代码在事件循环中，建议在新代码中使用 search_async。
    ...
    async def search_async(self, query: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            包含评分和描述的搜索结果
        """
        logger.info(f"异步 LLM 处理问题: {query[:DISPLAY_QUERY_TRUNCATE]}...")
        ...
        logger.info(f"异步 LLM 答案生成完成 (评分: {score}/10)")
        return [result]
...
请返回JSON格式的回答："""
    ...
    def get_provider_name(self) -> str:
        """获取提供者名称。"""
        return self.provider_name or "llm_api_async"

    # ========== 异步 API 调用辅助函数 ==========

    async def _send_api_request_async(self, prompt: str) -> str:
        """发送异步 API 请求并返回响应内容。
        ...
        logger.debug(f"发送异步请求到 {base_url} (超时: {timeout}s, 模型: {self.model})")
        ...
        try:
            ...
            logger.info(f"{self.provider_name} 异步 API返回内容长度: {len(content)} 字符")
            return content
        except httpx.HTTPStatusError as e:
            logger.error(f"异步 API 请求失败 (状态码: {e.response.status_code}): {e}")
            raise
        except httpx.RequestError as e:
            logger.error(f"异步 API 网络错误: {e}")
            raise
    ...
    async def _call_llm_api_async(
        ...
        for retry in range(max_retries):
            try:
                logger.info(
                    f"正在异步调用 {self.provider_name} API... (尝试 {retry + 1}/{max_retries})"
                )

                # 发送异步 API 请求
                content = await self._send_api_request_async(prompt)
                ...
                if retry < max_retries - 1:
                    ...
                    logger.warning(
                        f"异步 API请求失败（第{retry + 1}次尝试）: {e}，{wait_time}秒后重试..."
                    )
                    ...
                    # 所有重试都失败，抛出异常
                    logger.error(
                        f"{self.provider_name} 异步 API请求失败，已重试{max_retries}次: {e}"
                    )
                    raise Exception(f"异步 API请求失败，已重试{max_retries}次: {str(e)}")

            except (json.JSONDecodeError, ValueError, TypeError, KeyError, AttributeError) as e:
                # JSON解析或其他数据处理错误，进行重试
                if retry < max_retries - 1:
                    logger.warning(
                        f"异步数据处理错误（第{retry + 1}次尝试）: {e}，{retry + 1}秒后重试..."
                    )
                    await asyncio.sleep(retry + 1)
                    continue
                else:
    ...
    async def batch_search_async(self, queries: List[str]) -> List[List[Dict[str, Any]]]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(f"查询 {queries[i][:50]}... 处理失败: {result}")
                # 返回空结果
                processed_results.append([])
            else:
                # result 是 List[Dict[str, Any]] 类型
